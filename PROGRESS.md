...
- [~] **RPTEND** (DrawObject.cp DrawRPTEND): Repeat barlines (RPT_L/R/LR) already render via bar_line(). Remaining subtypes (RPT_DC, RPT_DS, RPT_SEGNO1/2) are punted — OG code also logs errors for these in GetRptEndDrawInfo. No test fixtures use these subtypes.
